...
1. Read the normative [contract v1](contract-v1.md).
2. Use the [JSON Schema](../schema/v1/contract.schema.json) for individual record
   shapes.
3. Use `getbiblesword-v1 validate` for ordering, framing, byte-envelope and footer
   rules that JSON Schema cannot express.
4. Review [architecture](architecture.md) and the
...
the output contract. These values are independent: release `0.2.0` emits
